    ...
    public static void main(String[] args) {
        try {
            ...
            while (input.hasNextLine()) {
                ...
                if(skips.contains(line))  //catches skipped lines
                    continue;
                Vector<String> info = new Vector<String>(Arrays.asList(line.split(cvsSplitBy)));
                if (line.startsWith(";;;;;;;")) {
                    Relation lastRelation = (Relation)db.getRelationMap().values().toArray()[db.getRelationMap().size()-1];
                    Attribute day = ((Tuple)lastRelation.getTuples().values().toArray()[lastRelation.getTuples().size()-1]).get("Days");
                    Attribute time = ((Tuple)lastRelation.getTuples().values().toArray()[lastRelation.getTuples().size()-1]).get("Time");
                    day.setValue(day.getValue() + " " + info.get(7));
                    time.setValue(time.getValue() + " " + info.get(8));
                    continue;
                }
                    ...
                    for(int i = 0; i < info.size(); i++) {
                        if(domains.get(i).getType() == Integer.class) {
                            ...
                            if(info.get(i) == null){
                                ...
                                if(i == 15 && info.get(i).contains("Distance")) {
                                    attrList.get(i).setValue("Distance");
                                }
                            }
                        }
                    }
