...
public class GrammarVisitorRelation extends GrammarBaseVisitor<Relation> {
    ...
    public Relation visitSelection(GrammarParser.SelectionContext ctx) {
        ...
        for (int i = 0; i < condition.size(); i += 4) {
            ...
            Attribute attrComp;
            if (condition.get(i).matches("^-?\\d+$")) {
                Domain<?> domain = new Domain<Integer>(Integer.class, condition.get(i));
                attrComp = new Attribute(domain, Integer.parseInt(condition.get(2))); //attribute_name
            }
            else {
                Domain<?> domain = new Domain<String>(String.class, condition.get(i));
                attrComp = new Attribute(domain, condition.get(i+2)); //attribute_name
            }

            Relation relation = db.selection(atomicExpr.getRelation(), attrComp, condition.get(i+1), condition.get(i+2));
            relations.add(relation);
        }

        Relation relation = new Relation("query");
        if (relations.size() == 2) {
            for (Tuple t : relations.get(0).getTuples().values()) {
                for (Tuple t2 : relations.get(1).getTuples().values()) {
                    if (t.equal(t2)) {
        ...
        relation.setDomainMap(relations.get(0).getDomainMap());
        db.getRelationMap().put("query", relation);
